<x-app-layout>
    <div class="max-w-2xl mx-auto pt-6 px-4 sm:px-6 lg:px-8 flex justify-end">
        <button id="theme-toggle" class="p-2 bg-gray-200 dark:bg-gray-800 rounded-lg transition-all border border-gray-300 dark:border-gray-600 shadow-sm">
            <span id="theme-toggle-icon">🌙</span>
        </button>
    </div>

    <div class="max-w-2xl mx-auto p-4 sm:p-6 lg:p-8">
        <form method="POST" action="{{ route('chirps.store') }}">
            @csrf
            <textarea
                name="message"
                placeholder="{{ __('What\'s on your mind?') }}"
                class="block w-full border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm dark:bg-gray-900 dark:text-gray-100 dark:border-gray-700"
            >{{ old('message') }}</textarea>
            <x-input-error :messages="$errors->get('message')" class="mt-2" />
            <x-primary-button class="mt-4">{{ __('Chirp') }}</x-primary-button>
        </form>

        <div class="mt-6 bg-white dark:bg-gray-800 shadow-sm rounded-lg divide-y dark:divide-gray-700">
            @foreach ($chirps as $chirp)
                <div class="p-6 flex space-x-2">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-gray-600 dark:text-gray-400 -scale-x-100" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
                    </svg>
                    <div class="flex-1">
                        <div class="flex justify-between items-center">
                            <div>
                                <span class="text-gray-800 dark:text-gray-200">{{ $chirp->user->name }}</span>
                                <small class="ml-2 text-sm text-gray-600 dark:text-gray-400">{{ $chirp->created_at->format('j M Y, g:i a') }}</small>
                            </div>
                        </div>
                        <p class="mt-4 text-lg text-gray-900 dark:text-gray-100">{{ $chirp->message }}</p>
                    </div>
                </div>
            @endforeach
        </div>
    </div>

    <script>
        const themeToggle = document.querySelector('#theme-toggle');
        const icon = document.querySelector('#theme-toggle-icon');
        const storageKey = 'theme-preference';

        const applyTheme = () => {
            const isDark = localStorage.getItem(storageKey) === 'dark';
            document.documentElement.classList.toggle('dark', isDark);
            icon.innerText = isDark ? '☀️' : '🌙';
        };

        // Aplica ao carregar
        applyTheme();

        themeToggle.addEventListener('click', () => {
            const isDark = localStorage.getItem(storageKey) === 'dark';
            localStorage.setItem(storageKey, isDark ? 'light' : 'dark');
            applyTheme();
        });
    </script>
</x-app-layout>
